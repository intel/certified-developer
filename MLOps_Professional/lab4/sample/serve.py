...
async def train(payload:TrainPayload):
    ...
    """
    model = HarvesterMaintenance(payload._model_name)
    ...
    model.save(payload._model_path)
...
async def predict(payload:PredictionPayload):
    ...
    results = inference(model_name = payload._model_name, stage = payload.stage,
                        model_run_id = payload._model_run_id, scaler_file_name = payload.scaler_file_name, 
