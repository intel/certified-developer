...
def inference(model_run_id: int, scaler_file_name: str, scaler_destination: str,
    ...
    data = data.astype({'Motor_Current': 'float64',
    ...
    zeroes_dataframe = pd.DataFrame(
        0, index=np.arange(1), columns=column_names)
    merged_df = pd.merge(zeroes_dataframe, processed_sample,
                         on=processed_sample.columns.tolist(), how='right').fillna(0)
