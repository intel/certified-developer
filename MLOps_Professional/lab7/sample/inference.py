...
def inference(
    ...
    if not d4p_destination.startswith(SAFE_BASE_DIR) or not d4p_file_path.startswith(
        SAFE_BASE_DIR
    ):
    ...
    # retrieve d4p model
    mlflow.artifacts.download_artifacts(
        run_id=model_run_id, artifact_path=d4p_file_name, dst_path=d4p_destination
    )
