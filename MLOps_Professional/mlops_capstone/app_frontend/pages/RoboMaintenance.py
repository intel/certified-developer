...
with app_tab:
    ...
    model_name = st.text_input('Model Name', key='model name',
                               help='The name of the model without extensions', value='model')
    model_path = st.text_input('Model Save Path', key='model path',
                               help='Provide the path without file name', value='./')
    test_size = st.slider('Percentage of data saved for Testing',
    ...
    mlflow_tracking_uri = st.text_input(
        'Tracking URI', key='uri', value='/home/ubuntu/certified-developer/MLOps_Professional/mlops_capstone/store/models/robot_maintenance')
    mlflow_new_experiment = st.text_input('New Experiment Name', key='new exp')
    mlflow_experiment = st.text_input(
        'Existing Experiment Name', key='existing exp')
    ...
    model_name = st.text_input(
        'Model Name', key='model name option', value='model')
    stage = manufacturer = st.selectbox(
        'Model Stage', options=['Staging', 'Production'])
    model_run_id = st.text_input('Run ID', key='model id')
    scaler_file_name = st.text_input(
        'Scaler File Name', key='scalar file', value='model_scaler.joblib')
    scaler_destination = st.text_input('Scaler Destination', key='scalerdest',
                                       value='/home/ubuntu/certified-developer/MLOps_Professional/mlops_capstone/store/outputs/robot_maintenance')
    ...
    with col21:
        ...
        age = st.number_input('Robot Age', min_value=0,
                              max_value=25, step=1, value=0)

    with col22:
        temperature = st.number_input(
            'Temperature', min_value=50, max_value=300, step=1)
        motor_current = st.number_input(
            'Motor Current', min_value=0.00, max_value=10.00, step=.05, value=5.00)
        lubrication_type = st.selectbox(
    ...
    with col23:
        ...
        num_repairs = st.number_input(
            'Repair Counts', min_value=0, max_value=50, step=1)
        product_assignment = st.selectbox(
...
with help_tab:
    ...
    st.markdown(
        "- Manufacturer: Provide the name of the manufacturer of the robotic arm")
    st.markdown(
        "- Model: Specify the model or specific type of the robotic arm. ")
    st.markdown(
